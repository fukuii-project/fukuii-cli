...
/** Runs one state fixture: seeds the pre-state, admits the transaction, settles
  ...
  * own: which files to read, which fork's expectations to read them under, and
  * how a divergence is described. How the corpus's vocabulary for a refusal
  * maps onto this build's is [[TransactionRefusalVocabulary]]'s, which the
  * published block tier reads as well.
  *
