...
# INSIDE THE REPOSITORY, and that is load-bearing rather than tidy: this script
# resolves its own repository root from its own location, so a mutant written to
# a temporary directory can fail for a missing tree instead of for the mutation.
# Both mutant arms below did exactly that once.
